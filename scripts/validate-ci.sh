...
# 1.5 Check for keyless Lazy item slots (issue #224) — delegated to scripts/check-lazy-keys.sh
# so the SAME guard runs here AND in the GitHub Actions `validate` job (#302). The non-aborting
# ERRORS=$((...)) form keeps a finding from short-circuiting the rest of the run.
echo -e "${YELLOW}[1.5] Checking for keyless Lazy item slots...${NC}"
if ! ./scripts/check-lazy-keys.sh app/src/main/java; then
    ERRORS=$((ERRORS + 1))
fi
echo ""
