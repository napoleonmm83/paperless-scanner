...
# 1.5 Check for keyless Lazy item slots (issue #224)
# Lazy* item{}/items(...) without a key= regress animation/recomposition (see #99).
# Scope-aware awk block scanner: a left word-boundary excludes non-Lazy overloads
# (DropdownMenuItem, NavigationBarItem, addItem, .item(...) ...); comment/KDoc lines
# are stripped so prose never trips it; multi-line calls are paren-balanced. Uses the
# non-aborting ERRORS=$((...)) form so a finding fails the summary gate without set -e
# short-circuiting the rest of the run. NUL-delimited grep + mapfile array pass filenames
# safely (GNU grep + bash 4.4+ — both present in CI ubuntu and Win11 Git-Bash).
echo -e "${YELLOW}[1.5] Checking for keyless Lazy item slots...${NC}"
LAZY_KT_FILES=()
mapfile -d '' -t LAZY_KT_FILES < <(grep -rlEZ '(^|[^[:alnum:]_.])(item|items|stickyHeader)[[:space:]]*[({]' app/src/main/java --include="*.kt" 2>/dev/null || true)
KEYLESS=""
...
if [ -n "$KEYLESS" ]; then
    echo -e "${RED}    ✗ Lazy item/items slot without key= found (issue #224):${NC}"
    echo "$KEYLESS"
    echo -e "${RED}        Add a stable key= argument (e.g. items(data, key = { it.id })).${NC}"
    ERRORS=$((ERRORS + 1))
else
    echo -e "${GREEN}    ✓ All Lazy item slots have keys${NC}"
fi
echo ""
